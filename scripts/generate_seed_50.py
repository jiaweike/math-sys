...
root = Path(r"E:\projects\math-sys")
out = root / "data" / "processed" / "seed_ingest_50.json"
...
theorems = [
    ...
    ("Fermat Little Theorem", "a^(p-1) = 1 mod p", "Use permutation of nonzero residues modulo p.", "p prime, p not divide a", "number-theory"),
    ...
    ("Inclusion-Exclusion Principle", "|A union B| = |A|+|B|-|A intersect B| (generalized).", "Count overlaps with alternating sums.", "Finite sets", "combinatorics"),
]
...
def mk_theorem(item):
    name, stmt, proof, cond, tags = item
    return {
        "name": name,
        "aliases": "",
...
def mk_formula(item):
    name, latex, meaning, constraints = item
    return {
        "name": name,
        "latex": latex,
        "meaning": meaning,
...
payload = {
    "theorems": [mk_theorem(x) for x in theorems],
    "formulas": [mk_formula(x) for x in formulas],
}
